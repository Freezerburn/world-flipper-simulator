...
class WorldFlipperData:
    def __init__(self, data_dir):
        ...
        with open(f"{data_dir}/character/character.json", "r", encoding="utf-8") as f:
            character_json = ujson.load(f)
        # Holds a bunch of user-facing description text, most importantly for our purposes: The name.
        with open(f"{data_dir}/character/character_text.json", "r", encoding="utf-8") as f:
            character_text_json = ujson.load(f)
        ...
        # for that character.
        with open(f"{data_dir}/character/character_status.json", "r", encoding="utf-8") as f:
            character_status_json = ujson.load(f)
        # Action skills use the "internal name" of a character as the key to access
        # their info.
        with open(f"{data_dir}/skill/action_skill.json", "r", encoding="utf-8") as f:
            action_skill_json = ujson.load(f)
        with open(f"{data_dir}/ability/ability.json", "r", encoding="utf-8") as f:
            abilities_json = ujson.load(f)
